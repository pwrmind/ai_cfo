...
class AIClassifier:
    def __init__(self):
        print("\n[Система] Загрузка нейросети (LaBSE) для семантического анализа...")
        # Модель загрузится один раз и будет работать локально
        self.model = SentenceTransformer("sentence-transformers/LaBSE")
        
        # ЭТАЛОННЫЕ ВЕКТОРЫ (СЕМАНТИЧЕСКИЕ ЯКОРЯ)
        self.categories = {
            "OPERATING_INCOME": [
                "Оплата от клиента за услуги", "Поступление выручки", "Оплата по счету за товар", 
                "Аванс от покупателя", "Розничная выручка эквайринг"
            ],
            "OPERATING_EXPENSE": [
                "Оплата поставщику за товар", "Закупка материалов", "Логистические услуги", 
                "Комиссия банка РКО", "Сдельная оплата", "Топливо ГСМ"
            ],
            "FIXED_EXPENSE": [
                "Аренда офиса помещения", "Зарплата сотрудников оклад", "Бухгалтерские услуги", 
                "Оплата интернет связи", "Подписка на сервисы софт хостинг", "Налоги и взносы"
            ],
            "CAPEX": [
                "Покупка оборудования", "Приобретение основных средств", "Строительство", 
                "Покупка автомобиля", "Компьютеры и техника"
            ],
            "FINANCIAL_FLOW": [
                "Перевод собственных средств", "Пополнение счета", "Выдача кредита", 
                "Заемные средства", "Возврат займа"
            ]
        }
    ...
    def classify(self, df):
        print("[Система] Анализ назначения платежей...")
        descriptions = df['Назначение'].tolist()
        ...
        for vec in vectors:
            ...
            # Если уверенность низкая, помечаем как прочее
            if max_sim < 0.35:
                best_cat = "OTHER"
